...
pal_ChIP <- c("#767676FF", "#800000FF", "black", "#155F83FF")
```
...
```{r}
# Targeted
mdat <- read_excel("data/ChIP/2023_0427_Exp33_targeted_ChIP.xls",
                 sheet = 'Results', 
                 range = "A38:AK377", 
                 trim_ws = T)
...
#Y <- Y |> filter(!CT > 30, !`Ct SD` > 0.90) # high CT is expected for non-amplifying controls

# Make gene name factor
Y$`Target Name` <- as.factor(Y$`Target Name`)

# Create grouping variable
Y$Condition <- gsub("_", " ", Y$`Sample Name`)
Y$Condition <- gsub("6h ", "", Y$Condition)
Y$Condition <- gsub("[[:digit:]]+ ", "", Y$Condition)
Y$Condition <- gsub(" ", "_", Y$Condition)

Y$Temp <- factor(gsub("_.*", "", Y$Condition))
Y$Sample <- factor(gsub(".*\\_", "", Y$Condition))
Y$Num <- factor(gsub("_.*", "", Y$`Sample Name`))

Y$Num <- factor(gsub("5", "4", Y$Num))
Y$Num <- factor(gsub("6", "4", Y$Num))

Y$row <- rownames(Y)

Y$Condition <- factor(Y$Condition, 
                      levels = c("RT_IgG", "RT_input", "RT_Tfeb",
                                 "Cold_IgG", "Cold_input",  "Cold_Tfeb")
                      )

# Normalize to each samples to temperature inputs
Y <- Y |> group_by(`Target Name`, Y$Temp, Y$Num) |>
  mutate(norm_quant = Quantity/Quantity[Sample=="input"])

Y <- Y |> filter(!Condition %in% c("RT_input", "Cold_input")
                 ) |> droplevels()

Y <- Y |> group_by(`Target Name`, Y$Temp, Y$Num) |>
  mutate(norm_quant = Quantity/Quantity[Sample=="IgG"])

Y <- Y |> filter(!row %in% c(23, 59, 62, 113, 132, 198, 215))
...
for (i in 1:length(targets)) {
  ...
  p1 <- ggpar(p0, palette = pal_ChIP, title = temp$`Target Name`[1],  
...
CairoPDF(file = "output/Fig3/2023_0428_Exp33_targeted_ChIP_all.pdf", height = 10, width = 8)
  for (i in 1:length(targets)) {
    print(plot_list[[i]])
  }
dev.off()
...
```{r}
temp <- Y |> filter(Temp == "RT") |> droplevels()

temp <- temp |> filter(`Target Name` %in% c("Psap intron", "Ctsb intron", 
                                            "npc1 prom", "npc1 intron")
                       ) |>
  droplevels()

temp$`Target Name` <- factor(temp$`Target Name`, levels = c("Psap intron", "Ctsb intron", 
                                            "npc1 prom", "npc1 intron")
               ) 
temp <- temp |> droplevels()

p0 <- ggbarplot(temp, x = "Target Name", y = "norm_quant",
                  add = c("mean_sd", "dotplot"), 
                  color = "Sample", fill = "Sample", 
                  error.plot = "errorbar", 
                  ...
                  position = position_dodge(0.8), size = 0.10) + 
    scale_y_continuous(limits = c(0.00, 1.25*max(temp$norm_quant, na.rm = T)),
                       expand = c(0,0)
                       )
  
p1 <- ggpar(p0, palette = pal_ChIP, title = "Targeted ChIP-qPCR",  
              xlab = "Binding site", ylab = "Normalized RNA Abundance") +
    scale_color_manual(values = c("black", "black")
                       ) +
    theme_bw(base_size = 16, base_family = "Arial")
...
stat.test <- temp |>
    group_by(`Target Name`) |>
    t_test(norm_quant ~ Sample) |>
    adjust_pvalue(method = "none") |>
    add_significance("p")
  
stat.test$p.adj.signif <- stat.test$p.signif # If you don't run this, adjusted p-values plot
stat.test <- stat.test |> add_xy_position(fun = "max", "Target Name", dodge = 0.8)
...
CairoPDF(file = "output/Fig3/Fig3C_Exp33_targeted_ChIP.pdf", height = 8, width = 10)
    print(p2)
dev.off()
```
...
Based on Roberto's suggestions, throw Ctsa into main figure and move something out.

```{r}
Y1 <- Y |> select(`Sample Name`, `Target Name`, Condition:Sample,
                  norm_quant)

new <- Y1 |> filter(Temp == 'RT') |>
  select(`Sample Name`, Sample, `Target Name`, norm_quant) |>
  droplevels()

names(new)[3:ncol(new)] <- names(old)

all_RT <- rbind(old, new)

temp <- all_RT |> filter(Gene %in% c("Psap intron", "Ctsb intron", 
                                            "Ctsa", "npc1 intron"))

temp$`Target Name` <- factor(temp$Gene, levels = c("Ctsa", "Ctsb intron",
                                                   "Psap intron", "npc1 intron")
               ) 
temp <- temp |> droplevels()

temp$Group <- ifelse(temp$Group == "TFEB", "Tfeb", temp$Group)
temp$Group <- factor(temp$Group, levels = c("IgG", "Tfeb")
                     )


p0 <- ggbarplot(temp, x = "Target Name", y = "Normalized",
                  add = c("mean_sd", "dotplot"), 
                  color = "Group", fill = "Group", 
                  error.plot = "errorbar", 
                  add.params = list(width = 0.2,
                  binwidth = 0.01*max(temp$Normalized, na.rm = T)
                  ),
                  position = position_dodge(0.8), size = 0.10) + 
    scale_y_continuous(limits = c(0.00, 1.25*max(temp$Normalized, na.rm = T)),
                       expand = c(0,0)
                       )
...
CairoPDF(file = "output/Fig3/Fig3C_Exp33_targeted_ChIP_updated.pdf", height = 8, width = 10)
    print(p2)
dev.off()
```
...
```{r}
temp <- all_RT |> filter(Gene %in% c("Cers1", "Plin2", "Insig2", "npc1 prom")
                         )

temp$`Target Name` <- factor(temp$Gene, levels = c("Cers1", "Plin2", "Insig2", "npc1 prom")
               ) 
temp <- temp |> droplevels()
...
CairoPDF(file = "output/Fig3/SFig3C_Exp33_targeted_ChIP_updated.pdf", height = 10, width = 14)
    print(p2)
dev.off()
```

## RT versus Cold
Pla2g15 promoter and intron in RT versus cold, IgG versus TFEB (?)
```{r}
Y1 <- Y |> filter(`Target Name` %in% c("pla2g15 prom", "pla2g15 intron")) |>
  droplevels()

Y1 <- Y |> filter(`Target Name` == "pla2g15 prom") |>
  droplevels()

Y1 <- Y1 |> filter(!row %in% c(120, 134, 140))

Y1$Condition <- factor(Y1$Condition, levels = c("RT_IgG", "RT_Tfeb",
                                                "Cold_IgG", "Cold_Tfeb")
                       )
Y1$Temp <- factor(Y1$Temp, levels = c("RT", "Cold")
                       )

p0 <- ggbarplot(Y1, x = "Temp", y = "norm_quant",
                  add = c("mean_sd", "dotplot"), 
                  color = "Condition", fill = "Condition", 
                  error.plot = "errorbar", 
                  add.params = list(width = 0.2,
                  binwidth = 0.01*max(Y1$norm_quant, na.rm = T)
                  ),
                  position = position_dodge(0.8), size = 0.10) + 
...
stat.test$p.adj.signif <- stat.test$p.signif # If you don't run this, adjusted p-values plot
stat.test <- stat.test |> add_xy_position(fun = "max", "Temp", dodge = 0.8)
...
CairoPDF(file = "output/Fig5/Fig5B_Exp33_targeted_ChIP_pla_prom.pdf", height = 14, width = 10)
    print(p2)
dev.off()
```
