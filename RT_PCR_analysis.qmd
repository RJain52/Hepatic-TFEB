...
CairoPDF(file = "output/Fig4/2023_0421_Exp45_KD_lyso_all.pdf", height = 10, width = 8)
  for (i in 1:length(targets)) {
    print(plot_list[[i]])
  }
dev.off()
...
# Something is wrong with sample 2
...
# Remove values with CT >30, CT SD > 0.90 [prefer 0.60 but data dependent]
Y <- Y |> filter(!CT > 30, !`Ct SD` > 0.90, !Quantity == 0) # For excel_02, set CT >29.4
...
CairoPDF(file = "output/Fig2/2023_0419_Exp33_Sylwia_03_all.pdf", height = 10, width = 8)
  for (i in 1:length(targets)) {
    print(plot_list[[i]])
  }
dev.off()
