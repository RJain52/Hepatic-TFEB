...
# TFEB OE Validation
Y <- Y |> filter(`Target Name` %in% c("hTFEB", "RPS3"), Condition %in% c("LacZ", "TFEB OE")) |>
  droplevels()
...
# for mdat
Y$Condition <- factor(Y$Condition, 
                      levels = c("LacZ", "TFEB OE")
                      )


p0 <- ggbarplot(Y, x = "Condition", y = "norm_quant",
                  add = c("mean_sd", "dotplot"), 
                  color = "Condition", fill = "Condition", 
                  error.plot = "errorbar", 
                  add.params = list(width = 0.2,
                  binwidth = 0.015*max(Y$norm_quant, na.rm = T)),
                  position = position_dodge(0.8), size = 0.10) + 
    scale_y_continuous(limits = c(0.00, 1.3*max(Y$norm_quant, na.rm = T)),
                       expand = c(0,0)
                       ) 
  
p1 <- ggpar(p0, palette = pal_cells[c(3,4)], title = "Hepa1-6 hTfeb",  
              xlab = "", ylab = "Normalized RNA Abundance") +
    scale_color_manual(values = c("black", "black")
                       ) +
    theme_bw(base_size = 16, base_family = "Arial")
...
CairoPDF(file = "output/Fig5/SFig5A_Hepa_TFEB_OE_qPCR.pdf",
         height = 8, width = 8)
  print(p2)
dev.off()
```
...
# Subset out the relevant data; choose which mdat here
Y <- mdat2 |> select(`Sample Name`, `Target Name`, 
                    `CT`, `Quantity Mean`,
                    `Ct SD`, `Quantity`)   
...
Y <- Y |> filter(`Target Name` %in% c("M6PR", "RPS3"), Condition %in% c("nt", "M6PR")) |>
  droplevels()
...
p1 <- ggpar(p0, palette = pal_cells[c(1,2)], title = "Hepa1-6 M6pr",  
              xlab = "", ylab = "Normalized RNA Abundance") +
    scale_color_manual(values = c("black", "black")
                       ) +
    theme_bw(base_size = 16, base_family = "Arial")
...
CairoPDF(file = "output/Fig5/SFig5C_Hepa_siM6pr_qPCR.pdf",
         height = 8, width = 8)
  print(p2)
dev.off()
```

## Pla2g15 KD validation

```{r}
mdat <- read_excel("data/qPCR/2023_0222 Pla2g15 knockdown confirmation.xls",
                 sheet = 'Results', 
                 range = "A38:AK377", 
                 trim_ws = T)
...
mdat$`Sample Name` <- gsub("siRNA ", "", mdat$`Sample Name`)
...
Y <- Y |> filter(`Target Name` %in% c("Pla2g15", "RPS3")) |>
  droplevels()
...
p1 <- ggpar(p0, palette = pal_cells[c(1,2)], title = "Hepa1-6 Pla2g15",  
              xlab = "", ylab = "Normalized RNA Abundance") +
    scale_color_manual(values = c("black", "black")
                       ) +
    theme_bw(base_size = 16, base_family = "Arial")
...
stat.test$p.adj.signif <- stat.test$p.signif
stat.test <- stat.test |>
  add_xy_position(fun = "max", x = "Condition", dodge = 0.8)
...
CairoPDF(file = "output/Fig5/SFig5E_Hepa_sipla2g15_qPCR.pdf",
         height = 8, width = 8)
  print(p2)
dev.off()
```
