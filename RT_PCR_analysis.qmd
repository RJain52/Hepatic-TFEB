...
```{r}
# eWAT Tfeb
mdat <- read_excel("data/qPCR/2023_0501_Exp45_eWAT_TFEB.xls",
                 sheet = 'Results', 
                 range = "A38:AK377", 
                 trim_ws = T)
...
```{r}
# BAT Tfeb
mdat <- read_excel("data/qPCR/2023_0508_Exp45_BAT_TFEB.xls",
                 sheet = 'Results', 
                 range = "A38:AK377", 
                 trim_ws = T)

# BAT TFEB Targets
mdat <- read_excel("data/qPCR/2023_0510_Exp45_BAT_targets.xls",
                 sheet = 'Results', 
                 range = "A38:AK377", 
                 trim_ws = T)
...
CairoPDF(file = "output/Fig4/2023_0510_Exp45_KD_BAT_targets_all.pdf", height = 10, width = 8)
  for (i in 1:length(targets)) {
    print(plot_list[[i]])
  }
dev.off()
...
## iWAT
This would need to be repeated due to too many outliers.
```{r}
# iWAT Tfeb
mdat <- read_excel("data/qPCR/2023_0510_Exp45_iWAT_TFEB.xls",
                 sheet = 'Results', 
                 range = "A38:AK377", 
                 trim_ws = T)
...
# Remove values with CT >30, CT SD > 0.90 [prefer 0.60 but data dependent]
Y <- Y |> filter(!CT > 30, !`Ct SD` > 0.90) # I didn't use this but should
...
CairoPDF(file = "output/Fig4/2023_0510_Exp45_KD_iWAT_all.pdf", height = 10, width = 8)
  for (i in 1:length(targets)) {
    print(plot_list[[i]])
  }
dev.off()
