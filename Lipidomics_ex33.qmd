...
p0 <- ggbarplot(mdat1, x = "Temp", y = "Total_PG",
                add = c("mean_sd", "dotplot"), fill = "Temp", 
                error.plot = "errorbar", 
                add.params = list(width = 0.35,
                                  binwidth = .025*max(mdat1$Total_PG, 
                                                      na.rm = T)
                                  ),
                alpha = 0.9, position = position_dodge(0.8), size = 0.15) +
  scale_y_continuous(limits = c(0.00, 1.15*max(mdat1$Total_PG, na.rm = T)
                                ),
                     expand = c(0,0)
                     ) 
p1 <- ggpar(p0, palette = pal_33, legend = "top",
            legend.title = "Group", title = "Liver total PG lipids",
            xlab = "Temperature",
...
CairoPDF(file = "output/Fig1/SFig1C_Liver_total_PG.pdf",
         height = 8, width = 8)
  print(p2)
dev.off()
```
...
p0 <- ggbarplot(mdat4, x = "Lipid", y = "value",
                ...
                group = "Temp", col= "Temp",
                error.plot = "errorbar", 
                add.params = list(width = 0.35, group = "Temp",
...
CairoPDF(file = "output/Fig1/Fig1E_BMP_species_high.pdf", height = 10, width = 14)
  print(p2)
dev.off()
...
CairoPDF(file = "output/Fig1/Fig1E_BMP_species_low.pdf", height = 10, width = 14)
  print(p2)
dev.off()
...
Split into low, medium and high abundance. Note that L06 seems to be a high outlier for PG lipids. The BMPs look ok. It might be an IS thing that was off there; I am keeping it in the dataset though as it changes sd but likely not the results of any t-tests.
