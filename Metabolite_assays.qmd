...
names(temp) <- c("Genotype", "ID", "Temp", "Initial", "Final")
...
p0 <- ggbarplot(data = temp, x = "weight", y = "g", 
                fill = "Condition", color = "Condition",
          add = c('mean_sd', 'dotplot'),
          add.params = list(width = 0.25,
                            binwidth = 0.05*min(temp$g,
                                               na.rm = T)
                            ),
          position = position_dodge(0.9)
          ) +
...
p1 <- ggpar(p0, palette = pal_45, title = "Exp 45",
            ylab = "Body weight (g)", xlab = "Time") 
...
stat.test <- stat.test |>
  na.omit() |>
  add_xy_position(fun = "max", "weight") 
...
# Connect dots for initial and final
