...
CairoPDF(file = "output/Fig3/SFig4X_Exp45_plasma_ACars.pdf",
         height = 8, width = 12)
  print(p0)
dev.off()
```
...
```{r}
mdat <- read_csv("eddie_ketones.csv")
mdat <- mdat |> filter(!`ID #` == 361)
mdat1 <- mdat |> select(`Mouse line`, Diet, Genotype, Sex, `Ketone Avg`)
mdat1$Diet <- gsub("Acute ", "", mdat1$Diet)
mdat1$Diet <- gsub(" Diet", "", mdat1$Diet)

mdat1$Condition <- paste0(mdat1$Sex, "_", mdat1$Diet)
mdat1$Condition <- factor(mdat1$Condition,
                          levels = c("F_Ctrl", "F_Keto",
                                     "M_Ctrl", "M_Keto")
                          )

mdat1$`Mouse line` <- factor(mdat1$`Mouse line`,
                          levels = c("ACAD10", "ACAD11")
                          )
mdat1$Genotype <- factor(mdat1$Genotype,
                          levels = c("WT", "KO")
                          )


p0 <- mdat1 |> filter(`Mouse line` == "ACAD11") |>
