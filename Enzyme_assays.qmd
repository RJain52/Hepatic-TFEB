...
# 4. Acid sphingomyelinase Assay

Exp 45

```{r}
mdat <- read_xlsx("data/Exp45/2023_0425_ASMase.xlsx", sheet = "FINAL")
mdat <- read_xlsx("data/Exp45/2023_0426_ASMase_at2.xlsx", sheet = "FINAL")

# remove samples 8 and 16 from first sheet
mdat <- mdat |> filter(!Sample %in% c(8, 15, 16))


# remove samples 5, 6, 18 and 21 from first sheet 
mdat <- mdat |> filter(!Sample %in% c(18, 21))
...
ASMase <- mdat # save for correlation 

mdat <- mdat |> mutate(RFU_ug = RFU_ug/mean(RFU_ug[Treatment == "Con_RT"]))

p0 <- ggbarplot(mdat, x = "Treatment", y = "RFU_ug", 
                add = c("mean_sd", "dotplot"), 
                color = "Treatment", fill = "Treatment", 
                add.params = list(width = 0.20, 
                                  binwidth = 0.075*min(mdat$RFU_ug, na.rm = T)
                                  ),
                position = position_dodge(0.8),
                size = 0.5) +
      scale_y_continuous(limits = c(0.00, 1.5*max(mdat$RFU_ug, na.rm = T)
                                    ),
                         expand = c(0,0)
                         )

p1 <- ggpar(p0, palette = pal_45, legend = "top", legend.title = "Group", 
            title = "ASMase activity assay",  
            xlab = "Group", ylab = "Relative activity (RFU/mg)") +
        theme_bw(base_size = 16, base_family = 'Arial') +
        theme(text = element_text(colour = 'black', 
                                  family = "Arial", 
                                  size = 16, 
...
CairoPDF(file = "output/Fig4/SFig4F_Exp45_ASMase.pdf", height = 10, width = 10)
  print(p2)
dev.off()
```
